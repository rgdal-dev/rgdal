...
We can enable on-demand download setting an internal PR$\phi$J variable; we see that with this setting, network download is enabled:

```{r, eval=run && odd_run}
enable_proj_CDN()
is_proj_CDN_enabled()
(shpr1 <- get_proj_search_paths())
```
...
```{r, eval=run && odd_run}
library(RSQLite)
if (file.exists(file.path(shpr1[1], "cache.db"))) {
db <- dbConnect(SQLite(), dbname=file.path(shpr1[1], "cache.db"))
