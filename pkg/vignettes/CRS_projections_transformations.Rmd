...
```{r, echo=FALSE}
mvrun <- FALSE
if (require(mapview, quietly=TRUE)) mvrun <- TRUE
```
...
```{r, eval=run}
library(RSQLite)
shpr <- strsplit(attr(getPROJ4libPath(), "search_path"), ":")[[1]]
...
```{r}
list_coordOps(wkt(b_pump), "EPSG:4326")
```
...
It is 117m West-North-West of the Ingestre Place position:
...
(b <- project(coordinates(b_pump), wkt(b_pump), inv=TRUE))
```
...
as we can see, the 1m accuracy point is 1.7m from the2m accuract point, just to the West:
