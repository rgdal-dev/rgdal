...
Using the default setting to remove the unintended `BOUNDCRS` representation, we can get what was (probably) intended (when the **sp** version is as was before this work-around was added, the following two chunck yield `BOUNDCRS` for some versions of PROJ/GDAL:
