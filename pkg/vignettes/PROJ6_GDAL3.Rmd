...
```{r}
if (packageVersion("sp") >= "1.3.3") cat(strwrap(gsub(",", ", ", (comment(crs)))), sep="\n")
```
...
The problem raised by the modernisation of PROJ can be encapsulated in this example of Scotland (once again the commented out runs are from a PROJ6/GDAL3 platform, the live runs from the platform building the vignette):
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_laea89), coordinates(pt1_lonlat_laea89)))) print(rbind(coordinates(pt0_laea89), coordinates(pt1_lonlat_laea89))) else TRUE
```
```{r}
list_coordOps(comment(slot(pt0_lonlat, "proj4string")), "EPSG:3035")
```

```{r}
list_coordOps(comment(slot(pt0_lonlat, "proj4string")), comment(CRS("+init=epsg:3035")))
```


```{r}
pt1_lonlat_laea89_1 <- spTransform(pt0_lonlat, CRS("+init=epsg:3035"))
get_last_coordOp()
```


```{r}
if (!isTRUE(all.equal(coordinates(pt0_laea89), coordinates(pt1_lonlat_laea89_1)))) print(rbind(coordinates(pt0_laea89), coordinates(pt1_lonlat_laea89_1))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_wgs32n), coordinates(pt1_lonlat_wgs32n)))) print(rbind(coordinates(pt0_wgs32n), coordinates(pt1_lonlat_wgs32n))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_psmerc), coordinates(pt1_lonlat_psmerc)))) print(rbind(coordinates(pt0_psmerc), coordinates(pt1_lonlat_psmerc))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_lonlat), coordinates(pt1_laea89_lonlat)))) print(rbind(coordinates(pt0_lonlat), coordinates(pt1_laea89_lonlat)))  else TRUE #EPSG:4326
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_wgs32n), coordinates(pt1_laea89_wgs32n)))) print(rbind(coordinates(pt0_wgs32n), coordinates(pt1_laea89_wgs32n))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_psmerc), coordinates(pt1_laea89_psmerc)))) print(rbind(coordinates(pt0_psmerc), coordinates(pt1_laea89_psmerc))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_lonlat), coordinates(pt1_wgs32n_lonlat)))) print(rbind(coordinates(pt0_lonlat), coordinates(pt1_wgs32n_lonlat))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_laea89), coordinates(pt1_wgs32n_laea89)))) print(rbind(coordinates(pt0_laea89), coordinates(pt1_wgs32n_laea89))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_psmerc), coordinates(pt1_wgs32n_psmerc)))) print(rbind(coordinates(pt0_psmerc), coordinates(pt1_wgs32n_psmerc))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_lonlat), coordinates(pt1_psmerc_lonlat)))) print(rbind(coordinates(pt0_lonlat), coordinates(pt1_psmerc_lonlat))) else TRUE #EPSG:4326
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_laea89), coordinates(pt1_psmerc_laea89)))) print(rbind(coordinates(pt0_laea89), coordinates(pt1_psmerc_laea89))) else TRUE
```
...
```{r}
if (!isTRUE(all.equal(coordinates(pt0_wgs32n), coordinates(pt1_psmerc_wgs32n)))) print(rbind(coordinates(pt0_wgs32n), coordinates(pt1_psmerc_wgs32n))) else TRUE
```
