...
setAs("GDALDataset", "SpatialGridDataFrame", asGDALROD_SGDF)

asSGDF_GROD <- function(x, at, region.dim, output.dim, ..., half.cell=c(0.5,0.5)) {
	if (!inherits(x, "GDALDataset"))
		stop("GDALDataset required")
	d = dim(x)
	if (missing(at)) at <- c(1, 1)
	...
	ysign <- sign(gt[6])
	co.x <- gt[1] + (at[2] - 1 + half.cell[2]) * cellsize[1]
	co.y <- ifelse(ysign < 0, gt[4] + (ysign*((output.dim[1] + 
		at[1] - 1) + (ysign*half.cell[1]))) * abs(cellsize[2]),
		gt[4] + (ysign*((at[1] - 1) + (ysign*half.cell[1]))) * 
		abs(cellsize[2]))
	cellcentre.offset <- c(x=co.x, y=co.y)
	grid = GridTopology(cellcentre.offset, cellsize, rev(output.dim))
	if (length(d) == 2)
...
readGDAL = function(fname, at, region.dim, output.dim, ..., half.cell=c(0.5,0.5), silent = FALSE) {
	if (nchar(fname) == 0) stop("empty file name")
	x = openDataset(fname)
	d = dim(x)
	if (missing(at)) at <- c(1, 1)
...
#	else {
		tds.out = createDataset(driver = d.drv, 
